...
class CodeChangeAnalyzer:
    def __init__(self, old_code: str, new_code: str):
        ...
        self.change_analysis: Optional[ChangeAnalysis] = None
    ...
    def analyze_changes(self) -> ChangeAnalysis:
        ...
        self.change_analysis = changes
        return changes

    def get_changed_functions_by_priority(self, min_priority: int) -> Set[str]:
        if self.change_analysis is None:
            raise ValueError(
                "You must call analyze_changes() before using this method.")

        changed_functions = set()

        # Check regular functions
        for func in self.change_analysis.changed_functions:
            if self._function_change_meets_priority(func, min_priority):
                changed_functions.add(func.name)

        # Check class methods
        for method in self.change_analysis.changed_class_methods:
            if self._class_method_change_meets_priority(method, min_priority):
                changed_functions.add(
                    f"{method.class_name}.{method.method_name}")

        # Add newly added and removed functions (considered highest priority)
        changed_functions.update(self.change_analysis.added_functions)
        changed_functions.update(self.change_analysis.removed_functions)

        return changed_functions

    def _function_change_meets_priority(self, change: FunctionChange, min_priority: int) -> bool:
        if change.signature_change and change.signature_change.priority >= min_priority:
            return True
        if change.nested_function_change and change.nested_function_change.priority >= min_priority:
            return True
        return any(c.priority >= min_priority for c in change.body_changes)

    def _class_method_change_meets_priority(self, change: ClassMethodChange, min_priority: int) -> bool:
        if change.signature_change and change.signature_change.priority >= min_priority:
            return True
        return any(c.priority >= min_priority for c in change.body_changes)
...
print("\nFunctions with changes of priority 5 or higher:")
high_priority_changes = analyzer.get_changed_functions_by_priority(5)
print(high_priority_changes)
